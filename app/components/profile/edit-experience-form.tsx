...
import { workExperienceFormSchema, type WorkExperienceFormData, type WorkExperienceFormValues } from "@/lib/schemas/work-experience-schema";
...
export const EditExperienceForm = forwardRef<HTMLFormElement, EditExperienceFormProps>(
  ({ experiences = [], onSave, onSubmitSuccess }, ref) => {
    ...
    const addTag = (index: number) => {
      if (newTag.trim()) {
        const currentValue = form.getValues(`experiences.${index}.experience_skill`);
        ...
        if (!trimmedSkills.includes(newTag.trim())) {
          ...
          form.setValue(`experiences.${index}.experience_skill`, newSkills.join(', '));
        }
        setNewTag("");
      }
    };
    ...
    const removeTag = (index: number, tagToRemove: string) => {
      ...
      form.setValue(`experiences.${index}.experience_skill`, filteredSkills.join(', '));
    };
    ...
    const handleSubmit = async (formData: WorkExperienceFormData) => {
      ...
      try {
        ...
        const transformedExperiences: WorkExperienceFormValues[] = formData.experiences.map(exp => ({
          ...exp,
        }));
                ...
                <div className="flex flex-col sm:flex-row justify-between mb-4">
                  <div className="flex items-start gap-3">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => removeExperience(index, (field as any).experience_id)}
                ...
                <div className="ml-0 sm:ml-13 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {form.watch(`experiences.${index}.experience_skill`)?.split(',')
                        .filter(Boolean)
                        .map((tag, tagIndex) => (
                          <Badge 
                            key={tagIndex} 
